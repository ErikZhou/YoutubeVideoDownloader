...
class DownloadSpider(scrapy.Spider):
    ...
    def __init__(self, type=0, target=None, **kwargs):
        # 播放列表，多个以空格切分
    ...
    def parse(self, response):
        ...
        for target_item in self.target:
            ...
            file_path = self.path_domain + '/' + play_list_name
            mkdir(file_path)
            for video in videos:
                ...
                if os.path.exists(file_path + '/' + video_name) == False:
                    ...
                    for format in formats:
                        if format['height'] == 720:
                            ...
                            print('正在下载第' + (str)(target_item_index) + '/' + (str)(total_target_item) + '个列表，第' + (str)(video_count) + '/' + (str)(total_video_count2) + '个视频 ' + play_list_name + ' ' + video_title)
                            downloadFile(video_real_url, file_path, video_name)
                            print((str)(target_item_index) + '/' + (str)(total_target_item) + '个列表，第' + (str)(video_count) + '/' + (str)(total_video_count2) + '个视频 -> ' + play_list_name + ' ' + video_title + ' -> 下载完成')
                            video_count += 1
                            total_video_count += 1
                            break
                else:
                    print((str)(target_item_index) + '/' + (str)(total_target_item) + '个列表，第' + (str)(video_count) + '/' + (str)(total_video_count2) + '个视频 -> ' + play_list_name + ' ' + video_title + ' -> 已下载')
                    video_count += 1
                    total_video_count += 1
            target_item_index += 1
